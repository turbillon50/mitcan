...
export default function SeguimientoClient({ folio }: { folio: string }) {
  ...
  useEffect(() => {
    load();
    const t = setInterval(load, 12000); // tiempo real (polling)
  ...
  if (!pedido) {
    return (
      <div className="flex flex-col gap-3 pb-6">
        ...
        <div className="card h-56 animate-pulse bg-surface-2" />
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-5 pb-6">
      ...
      <a href={`tel:${TEL_PEDIDOS}`} className="btn-ghost w-full">
        <Phone size={15} /> Llamar para pedir · {TEL_PEDIDOS_DISPLAY}
      </a>
    </div>
  );
}
...
function Encuesta({ pedidoId, onDone }: { pedidoId: number; onDone: () => void }) {
  ...
  const enviar = async () => {
    if (completo === null || estrellas === 0) {
      setError("Contesta si llegó completo y tu calificación.");
      return;
    }
    setLoading(true);
    setError(null);
    ...
    <section className="card flex flex-col gap-4 p-5">
        ...
        <div className="flex gap-2">
          {[{ v: true, l: "Sí" }, { v: false, l: "No" }].map(({ v, l }) => (
            <button
              ...
              className={`chip px-5 py-2.5 text-sm ${completo === v ? "chip-active" : ""}`}
            >
              {l}
            </button>
          ))}
      ...
      <div>
        ...
        <div className="flex gap-1.5">
          {[1, 2, 3, 4, 5].map((n) => (
            <button key={n} onClick={() => setEstrellas(n)} aria-label={`${n} estrellas`} className="p-1">
              <Star
                size={28}
                className={n <= estrellas ? "fill-accent text-accent" : "text-on-bg-muted/40"}
              />
            </button>
          ))}
        </div>
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="¿Algo que debamos mejorar?"
          value={comentarios}
