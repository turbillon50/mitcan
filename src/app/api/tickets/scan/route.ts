  ...
  try {
    ...
    if (forzar_invalido) {
      await prisma.$executeRawUnsafe(
        `UPDATE tickets SET estado='invalido', revisor_id=$1 WHERE id=$2`,
        user.id, ticket.id
      );
    ...
    const campo = nuevoEstado === "despachado" ? "cajero_id" :
                  nuevoEstado === "pagado" ? "cajero_id" : "revisor_id";
    const campoAt = nuevoEstado === "despachado" ? "caja_at" :
                    nuevoEstado === "pagado" ? "caja_at" : "salida_at";
    ...
    if (nuevoEstado === "entregado" && ticket.user_id) {
      ...
      await prisma.notificaciones.create({
        data: {
          ...
          mensaje: `Ticket ${numero} verificado en salida. ¡Mucha suerte!`,
          tipo: "sorteo",
        },
      }).catch(() => null);
    }
